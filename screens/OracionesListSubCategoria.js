...
function Index(props) {  
  ...
  {   
    ...
    let sub_categoria 
    if(props){
      sub_categoria = props.route.params.data.title_sub_categoria
    }else{
        sub_categoria = props.route.params.data.title_sub_categoria
    }
  ...
  //console.log(props.route.params.category, "categori")
   useEffect(()=>{
      ...
      console.log(base_url(Api,`get/oraciones/subcategoria/${props.route.params.data.id}`))
      axios.get( base_url(Api,`get/oraciones/subcategoria/${props.route.params.data.id}`)).then(function (response) {
        ...
        <ScrollView
          ...
          <View>
           ...
           <TouchableOpacity onPress={()=>goToScreen('Oraciones')} style={{marginTop: 30, marginLeft: 20}}>
              ...
              <Text style={{ marginLeft: 35, color: "#fff", fontSize: 20, marginBottom: 15}}>{props.route.params.data.title_sub_categoria}</Text>
               </ImageBackground>
            ...
            <View style={{marginBottom: 100}}>
                {
                    Oraciones.map((item, key) =>{
                        return <View style = {{marginTop: 10, padding: 18, width : "100%"}} key={key}>
